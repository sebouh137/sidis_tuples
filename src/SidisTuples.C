        ...
        if(hadron_tree != NULL || dihadron_tree != NULL || dipion_tree != NULL){
          for(int j =0; j<parts.size();j++){
            ...
            if((z > 0.4 && abs(h_pid)==211 && dihadron_tree!= NULL) || (dipion_tree != NULL && abs(h_pid)==211)){
              ...
              vector<int> matchedMCindicesH2 = {};
              for(int k = 0; k<parts.size();k++){ //loop on h2 candidates
                if(k == j)
                  continue;
                ...
                if(isMC){
                  ...
                  for(int kk = 0; kk<mcparts->getRows();kk++){
                    ...
                    if(abs(mc.Theta()-h2_th)>1*TMath::Pi()/180){
                      continue;
                    }
                    if(abs(angle(mc.Phi()-h2_ph))>3*TMath::Pi()/180){
                      continue;
                    }
                    //if(debug) cout << "passed phi" <<endl;
                  ...
                  if(kbest2 >=0){
                    matchedMCindicesH2.push_back(kbest2);
                    h2_truth_pid = mcparts->getPid(kbest2);
                    double mass = db->GetParticle(h2_truth_pid)->Mass();
                    h2_truth.SetXYZM(mcparts->getPx(kbest2),mcparts->getPy(kbest2),mcparts->getPz(kbest2), mass);
              ...
              if(isMC){ //now include unmatched h2 candidates.
                ...
                for(int kk = 0; kk<mcparts->getRows();kk++){
                  ...
                  if(alreadymatched)
                    continue;
                  //cout << "found an unmatched h2 candidate" << endl;
                  //if(debug) cout << "hadron" <<endl;
                  TVector3 mc(mcparts->getPx(kk),mcparts->getPy(kk),mcparts->getPz(kk));
